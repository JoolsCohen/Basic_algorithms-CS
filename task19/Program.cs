...
{
  Console.WriteLine($"А это {num} что за динозавр? Написано же - ПЯТЬ циферок");
}
else
{
  Palindrome(num);
  ...
  {
    if ((num / 10000 == num % 10) && ((num / 1000) % 10 == (num % 100) / 10)) Console.WriteLine($"Да, это {number} - Палиндром");
    else Console.WriteLine($"Нет, это {number} - не палиндром");
  }
}
